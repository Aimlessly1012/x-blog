...
function ReadingNewsCard({ item, isFirst = false }: { item: Article; isFirst?: boolean }) {
  ...
  return (
    <article className={`group ${isFirst ? 'pb-16 mb-12 border-b border-gray-800' : 'py-12'}`}>
      {/* Date */}
      ...
      {/* Title */}
      <h2 className={`font-bold text-gray-100 mb-4 group-hover:text-pink-400 transition-colors ${isFirst ? 'text-3xl md:text-4xl' : 'text-2xl md:text-3xl'}`}>
        {item.title || '无标题'}
      </h2>
    ...
    <div className="py-24 text-center">
      ...
      <div className="flex flex-wrap justify-center gap-3 text-sm">
        {['Claude 新功能', '实用技巧', '开发教程'].map((f, i) => (
          <span key={i} className="px-4 py-2 bg-gray-800 rounded-full text-gray-500">
            {f}
          </span>
        ))}
      </div>
        ...
        <div className="max-w-2xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">每个工作日 8:00 更新</p>
            </div>
